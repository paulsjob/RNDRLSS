...
import { ValidationAndDeployPanel } from './components/ValidationAndDeployPanel';
  ...
  return (
    <div className={`flex-1 h-full transition-all duration-500 ${isTruthMode ? 'bg-black' : isWiringMode ? 'bg-blue-900/5' : 'bg-zinc-950'} relative`} onDragOver={(e) => !isTruthMode && e.preventDefault()} onDrop={onDrop}>
      ...
      >
        ...
        {/* ITEM 42: Refactored Validation and Deploy Panel */}
        <Panel position="top-right" className={`transition-all duration-500 ${isTruthMode ? 'opacity-30 blur-[2px] pointer-events-none' : ''}`}>
          <ValidationAndDeployPanel />
        </Panel>
        ...
        {/* Deployment Backdrop (only if deploying) */}
        {deployment.status === 'deploying' && (
          <div className="absolute inset-0 z-[300] bg-zinc-950/60 backdrop-blur-sm flex items-center justify-center animate-in fade-in duration-500">
            <div className="w-full max-w-sm p-8 bg-zinc-900 border border-zinc-800 rounded-[2.5rem] shadow-[0_40px_100px_rgba(0,0,0,0.8)] flex flex-col items-center text-center animate-in zoom-in-95 duration-300">
              <div className="space-y-6 w-full">
                <div className="w-16 h-16 bg-blue-600/10 rounded-full border border-blue-500/20 flex items-center justify-center mx-auto relative">
                  <div className="absolute inset-0 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="text-blue-500"><path d="M12 2v8"/><path d="m16 6-4 4-4-4"/><rect width="20" height="8" x="2" y="14" rx="2"/></svg>
                </div>
                <div className="space-y-2">
                  <h3 className="text-lg font-black text-white uppercase tracking-widest">Deploying to Edge</h3>
                  <p className="text-[10px] text-zinc-500 font-bold uppercase">Optimizing Graph Logic...</p>
                </div>
